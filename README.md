...
- **No Chromium.** The core is pure Rust. The macOS shell uses Apple's WebKit as the rendering backend today, isolated behind a clean abstraction layer — and a real [Servo](https://github.com/servo/servo) v0.5.0 embedding already implements the same interface behind a feature flag, so the fully independent Rust engine can be swapped in without touching a single line of UI code.
...
│  WKWebView via FFI  │  │  Servo v0.5.0, pure Rust   │
└─────────────────────┘  └───────────────────────────-┘
           │
┌──────────▼──────────────────────────────────────────┐
│  blaze-adblock  (Rust)                              │
...
│   ├── blaze-engine-servo# Servo v0.5.0 embedding (--features servo) + simulated engine for parity tests
...
└── scripts/
    ...
    └── version.sh        # Automatic per-build semantic version
```

### Versioning

Every build versions itself — there is nothing to bump between builds. `scripts/version.sh` derives a unique [SemVer](https://semver.org) from the repository:

```
0.7.3+20260919131205.g58b3605
│ │ │  │              └ commit (".dirty" appended for uncommitted changes)
│ │ │  └ UTC build time — unique for every build
│ │ └ patch: commits since the base version was set in Cargo.toml
└─┴ major.minor: `[workspace.package] version` in Cargo.toml (bump by hand for a release line)
```

Xcode stamps it into the app on every build (About Blaze shows `0.7.3 (20260919.131205)`), and `build-dmg.sh` names the image after it, e.g. `Blaze-0.7.3+20260919131205.dmg`.
